...
*   **用户系统与历史记录**：
    *   🔐 **安全登录**: 集成 Logto 身份验证，原生支持微信扫码、手机号登录，非常适合国内用户。
...
*   **身份验证**: [Logto](https://logto.io/)
*   **数据库 & ORM**: [Supabase](https://supabase.com/) (PostgreSQL) + [Prisma](https://www.prisma.io/)

## 🔐 身份验证配置 (Logto)

本项目使用 Logto 进行用户认证。Logto 对国内用户非常友好，支持微信、短信验证码等：

1. 登录 [Logto Console](https://cloud.logto.io/)。
2. 创建一个 **Next.js (App Router)** 应用。
3. 在 **Sign-in experience** 中配置：
   - 启用 **Email address** 或 **Phone number**。
   - 配置 **Connectors** 接入阿里云/腾讯云短信，或集成 **WeChat (微信)** 扫码登录。
4. 将 App ID, App Secret, Endpoint 等信息填入 `.env`。
